...
async function main(): Promise<void> {
  ...
  // Disable experimental betas to avoid LiteLLM compatibility issues
  ...
  if (resolvedModel && resolvedModel.startsWith('gemini') && containerInput.secrets?.GOOGLE_API_KEY) {
    ...
    const proxyProc = spawn('uvx', ['--with', 'litellm[proxy]==1.61.0', 'litellm', '--port', port.toString(), '--drop_params'], {
